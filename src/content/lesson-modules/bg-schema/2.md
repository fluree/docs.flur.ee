...
For example, if we were creating a new `actor`, our transaction would contain `"_id": "actor"`. When creating a new subject, you can also add a [temporary id](/docs/transact/basics#temporary-ids) by prefixing the collection name with any invalid character (any character other than a-z, A-Z, 0-9, _) and the temporary id. 
