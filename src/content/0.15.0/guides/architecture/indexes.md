...
The immutability guarantee around the storage, combined with the Fluree's [Flake partitioning](/guides/architecture/flakes#flake-partitioning) strategy that helps minimize the number of affected index nodes new transactions typically need to revise, gives Fluree the ability to linearly scale its query servers while allowing them to effectively run at in-memory speeds -- even with a fraction of memory that the entire database consumes.
