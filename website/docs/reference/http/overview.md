...
Command | `/fdb/[NETWORK-NAME]/[DBID]/command` | Commands, such as transactions, with a signature in the body. See [signing transactions](../../concepts/identity/signatures#signed-transactions).
...
You need password authentication enabled to use these endpoints. See [config options](../../overview/start/installation#password-and-jwt-token-settings) for all password authentication options. See the [Password Management Guide](../../concepts/identity/password-management) for more information. For an implementation example refer to the [Comics Store](https://github.com/fluree/developer-hub) repo located in the Fluree Developer Hub.
...
More information on [signing queries](../../concepts/identity/signatures#signed-queries) and [signing transactions](../../concepts/identity/signatures#signed-transactions) can be found in the linked sections.

See [Downloaded Endpoint Examples](../../reference/http/examples) for examples of how to use each of the endpoints.
